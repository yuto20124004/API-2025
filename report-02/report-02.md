...
204:処理は成功したが返すコンテンツがない  
リダイレクト  
301:リソースが恒久的に移動した  
302:リクエストしたリソースのURLが一時的に変更  
304:キャッシュが有効でッ再取得の必要がない  
クライアントエラー  
400:リクエストが正しくない  
401:認証が必要  
404:指定したリソースが存在しない  
...
503:サ－バーが一時的に利用不可  
100番台はプロトコルの切り替えや巨大なデータの送信時の合図として使われる。200番台は通常の成功レスポンス（作成・取得・削除）。300番台はAPIのURL移転やCDNへのリダイレクト、キャッシュ制御。400番台はバリデーション、認証・認可、レート制限、存在しないリソースの報告。500番台はサーバー内部エラー、障害、外部サービスの問題の報告。に使われる。
...
・指定された時間までをキャッシュしてよいという有効期限を絶対時刻で示すヘッダ。Cache-Controlがない場合に利用されるが、同時に指定されている場合はCache-Controlが優先される  
ETag: "abc123"  
...
[  
  ...
  - 構造の特徴  
    レスポンスJSONは単一のオブジェクトで構成され、複数のキーと値のペアを持つフラットな構造。
